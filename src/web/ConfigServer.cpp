...
void ConfigServer::_handleGpio() {
    ...
    String body =
        ...
        "calRRef:10000,calBeta:3950,calR25:10000,calUnit:'',zoneSet:0,zoneHyst:0,"
        "pixelCount:30},it));"
        ...
        "let pixelPart='';"
        "if(it.type==='neopixel'){"
        "pixelPart=`<div style='flex-basis:100%;margin-top:2px;display:flex;gap:6px;align-items:center'>`"
        "+`<span class='rlbl'>LED</span>`"
        "+`<input type='number' min='1' max='500' style='width:80px' placeholder='кол-во' value='${it.pixelCount||30}' onchange='items[${i}].pixelCount=+this.value'>`"
        "+`<span style='font-size:.75rem;color:#8e8e93'>светодиодов в ленте</span>`"
        "+`</div>`;}"
        "let zonePart='';"
...
void ConfigServer::_handleDash() {
    String body =
        ...
        "if(p.type==='neopixel'){"
        "const hex=p.on?('#'+[p.r||0,p.g||0,p.b||0].map(v=>v.toString(16).padStart(2,'0')).join('')):'#1a1a1a';"
        "return `<div style='display:flex;align-items:center;gap:8px'>`"
        "+`<div style='width:36px;height:18px;border-radius:5px;background:${hex};border:1px solid #444'></div>`"
        "+`<span style='font-size:.82rem;color:#8e8e93'>${p.effect||'off'}</span>`"
        "+`</div>`;}"
        "return '?';}"
        "const NEO_EFFS=["
        "{v:'off',l:'Выключить'},{v:'static',l:'Статично'},{v:'blink',l:'Мигание'},"
        "{v:'breathe',l:'Пульс'},{v:'rainbow',l:'Радуга'},{v:'strobe',l:'Строб'},"
        "{v:'sunrise',l:'Рассвет'},{v:'wipe',l:'Заполнение'}];"
        "function btn(p){"
        ...
        "if(p.type==='neopixel'&&p.ok){"
        "const hex='#'+[p.r||0,p.g||0,p.b||0].map(v=>v.toString(16).padStart(2,'0')).join('');"
        "const effOpts=NEO_EFFS.map(e=>`<option value='${e.v}'${(p.effect||'off')===e.v?' selected':''}>${e.l}</option>`).join('');"
        "return `<div style='margin-top:8px'>`"
        "+`<div style='display:flex;gap:6px;align-items:center;margin-bottom:6px'>`"
        "+`<input type='color' value='${hex}' style='width:36px;height:28px;padding:0;border:none;cursor:pointer;border-radius:4px'`"
        "+` onchange=\"cmd('${p.key}',{effect:'static',r:parseInt(this.value.slice(1,3),16),g:parseInt(this.value.slice(3,5),16),b:parseInt(this.value.slice(5,7),16),brightness:${p.brightness||200}})\">`"
        "+`<input type='range' min='0' max='255' value='${p.brightness||200}' style='flex:1'`"
        "+` oninput=\"cmd('${p.key}',{on:true,r:${p.r||0},g:${p.g||0},b:${p.b||0},brightness:+this.value})\">`"
        "+`</div>`"
        "+`<div style='display:flex;gap:6px;flex-wrap:wrap'>`"
        "+`<select style='flex:1;min-width:110px' onchange=\"cmd('${p.key}',{effect:this.value})\">${effOpts}</select>`"
        "+`<button onclick=\"cmd('${p.key}',{effect:'rainbow',speed:80,brightness:200})\" `"
        "+`style='padding:4px 10px;font-size:.82rem;background:#2a2a2d;color:#a855f7;border-radius:7px;border:1px solid #555;cursor:pointer'>&#x1F308;</button>`"
        "+`</div></div>`;}"
        "return '';}"
