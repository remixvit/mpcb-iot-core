...
void ConfigServer::_handleGpio() {
    ...
    String body =
        ...
        "+`<select onchange='rules[${i}].trigger=this.value'>${periphOpts(r.trigger,TRIGGER_TYPES)}</select>`"
        ...
        "const tk=tr?san(tr.label)||tr.type+'_'+tr.pin:'';"
        "const gk=tg?san(tg.label)||tg.type+'_'+tg.pin:'';"
        "rules.push({trigger:tk,event:'pressed',action:'toggle',target:gk,pulseMs:500});"
        "renderRules();}"
